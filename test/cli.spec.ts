...
describe("CLI", () => {
  ...
  it("no args", done => {
    nexpect.spawn("node", [sw2dtsPath]).run((err, stdout, exit) => {
      assert(!!stdout.join(""));
      assert(!err);
      assert(exit === 0);
      done();
  ...
  it("invalid input path should be error", done => {
    nexpect.spawn("node", [sw2dtsPath, "invalid-path"]).run((err, stdout, exit) => {
  ...
  it("file to stdout", done => {
    ...
    nexpect.spawn("node", [sw2dtsPath, swaggerJsonPath]).run((err, stdout, exit) => {
  ...
  it("file to stdout with namespace", done => {
    ...
    nexpect.spawn("node", [sw2dtsPath, swaggerJsonPath, "-n", "foo"]).run((err, stdout, exit) => {
      const actual = stdout.join("\n") + "\n";
      const expected = fs.readFileSync(expectedPath).toString();
      assert.equal(actual, expected);
      assert(!err);
  ...
  it("stdin to stdout", done => {
    ...
    const expectedPath = path.resolve(fixturesDir, "swagger.json.d.ts");
    nexpect.spawn("sh", [testShell]).run((err, stdout, exit) => {
      const actual = stdout.join("\n") + "\n";
      const expected = fs.readFileSync(expectedPath).toString();
      assert.equal(actual, expected);
  ...
  it("file to out file", done => {
    ...
    nexpect.spawn("node", [sw2dtsPath, swaggerJsonPath, "--output", outputPath]).run((err, stdout, exit) => {
  ...
  it("stdin to out file", done => {
    ...
    const expectedPath = path.resolve(fixturesDir, "swagger.json.d.ts");
    const outputPath = `${testWorkingDir}/output.d.ts`;
    nexpect.spawn("sh", [testShell, outputPath]).run((err, stdout, exit) => {
      const actual = fs.readFileSync(outputPath).toString();
      const expected = fs.readFileSync(expectedPath).toString();
      assert.equal(actual, expected);
      assert(stdout.join("") === "", "stdout should be empty.");
  ...
  it("file to stdout with query params", done => {
    ...
    nexpect.spawn("node", [sw2dtsPath, "--with-query", swaggerJsonPath]).run((err, stdout, exit) => {
      const actual = stdout.join("\n") + "\n";
      const expected = fs.readFileSync(expectedPath).toString();
      assert.equal(actual, expected);
      assert(!err);
  ...
  it("file to stdout with --sort-props option", done => {
    const swaggerJsonPath = path.resolve(fixturesDir, "swagger.json");
    const expectedPath = path.resolve(fixturesDir, "swagger.json.sort-props.d.ts");
    nexpect.spawn("node", [sw2dtsPath, "--sort-props", swaggerJsonPath]).run((err, stdout, exit) => {
      const actual = stdout.join("\n") + "\n";
      const expected = fs.readFileSync(expectedPath).toString();
      assert.equal(actual, expected);
      assert(!err);
  ...
  it("file to stdout with -ws option", done => {
    const swaggerJsonPath = path.resolve(fixturesDir, "swagger.json");
    const expectedPath = path.resolve(fixturesDir, "sort-props.with-query-params.d.ts");
    nexpect.spawn("node", [sw2dtsPath, "-ws", swaggerJsonPath]).run((err, stdout, exit) => {
      const actual = stdout.join("\n") + "\n";
      const expected = fs.readFileSync(expectedPath).toString();
      assert.equal(actual, expected);
      assert(!err);
  ...
  it("file to stdout with query params and namespace", done => {
    ...
    nexpect.spawn("node", [sw2dtsPath, "--with-query", "-n", "foo", swaggerJsonPath]).run((err, stdout, exit) => {
      const actual = stdout.join("\n") + "\n";
      const expected = fs.readFileSync(expectedPath).toString();
      assert.equal(actual, expected);
      assert(!err);
